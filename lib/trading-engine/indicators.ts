...
// Map: cacheKey (symbol_timeframe_lastCandleTimestamp) -> Record<string, any>
...
function getCacheKey(candles: Candle[]): string {
  ...
  return `${last.timestamp}_${candles.length}`;
}
...
function getCached<T>(candles: Candle[], key: string, calculator: () => T): T {
  ...
  if (!itemCache) {
    ...
    if (indicatorCache.size > 100) {
      const keysToDelete = Array.from(indicatorCache.keys()).slice(0, 20);
